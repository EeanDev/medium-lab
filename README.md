...
This project creates a realistic CTF challenge where participants must analyze network traffic to find a hidden flag. The system sends packets sequentially over time:
- **Real flag**: `$flag{nahanapmo}` sent via UDP every 5 minutes (only when admin logged in)
- **Fake flags**: 12 confusing FLAG messages sent via ICMP/UDP every 25 seconds to all IPs
- **Noise traffic**: TCP/UDP/ping packets sent by separate noise generator every 2 seconds
...
2. **Look for patterns**:
   - Real flag: `$flag{nahanapmo}` appears every 5 minutes
   - Fake flags: Various `FLAG{...}` strings appear every 25 seconds to all IPs
   - Protocols: Only UDP and ICMP contain FLAG messages
   - Sequence: 12 fake flags over 5 minutes, then real flag

3. **Identify the flag**:
   - Look for the unique `$flag{...}` format (not `FLAG{...}`)
   - Check timing: exactly 5-minute intervals for real flag
   - Fake flags appear in 25-second intervals before real flag
